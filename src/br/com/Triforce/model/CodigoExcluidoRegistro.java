...
public class CodigoExcluidoRegistro implements Serializable {
    ...
    public void setPed(int codigo, int ped) {
        excluidos.get(codigo).setPed(ped);
    }
    ...
    public void setFunc(int codigo, int func) {
        excluidos.get(codigo).setFunc(func);
    }
    ...
    public void salvar() {
        arqExcluidoPer.salvarExcluidos(excluidos);
    }

    public int verificaMap(int cod) {
        if (excluidos.containsKey(cod)) {
            return 1;
